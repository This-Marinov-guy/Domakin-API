-- Refactor agents so sheet fields live as real columns and metadata only keeps sync details.
-- Keeps source_key and email as technical columns for upsert + unique email enforcement.
...
-- Preserve old technical columns inside metadata before dropping them.
DO $$
BEGIN
    IF EXISTS (
...
-- Drop old extracted columns before rebuilding sheet columns from agents.data.
-- If the sheet has Name/Phone columns, they will be recreated below from exact sheet data.
ALTER TABLE agents
...
-- Flatten existing agents.data JSON keys into normalized real columns.
DO $$
DECLARE
    data_key text;
    data_keys text[];
...
-- Remove legacy source key prefixes: email:{hash}, phone:{hash}, row:{hash} -> {hash}.
UPDATE agents
SET source_key = regexp_replace(source_key, '^(email|phone|row):', '')
WHERE source_key ~ '^(email|phone|row):';

-- Keep only the requested metadata keys.
UPDATE agents
